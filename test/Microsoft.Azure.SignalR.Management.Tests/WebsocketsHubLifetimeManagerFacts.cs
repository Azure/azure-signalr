    ...
    public class WebsocketsHubLifetimeManagerFacts
    {
        ...
        public async Task RefreshConnectionAuthAsync_NullOrEmptyConnectionToken_ThrowsArgumentException()
        {
            ...
            var exception = await Assert.ThrowsAsync<ArgumentException>(
                () => manager.RefreshConnectionAuthAsync(null!, DateTimeOffset.UtcNow, null, default));
            Assert.Equal("connectionToken", exception.ParamName);

            exception = await Assert.ThrowsAsync<ArgumentException>(
                () => manager.RefreshConnectionAuthAsync(string.Empty, DateTimeOffset.UtcNow, null, default));
            Assert.Equal("connectionToken", exception.ParamName);
        }

        [Fact]
        public async Task RefreshConnectionAuthAsync_DelegatesTokenKeyedMessageToContainer_ReturnsResult()
        {
            var manager = CreateManager();
            ...
            var result = await manager.RefreshConnectionAuthAsync("conn-token-1", expireTime, new[] { new Claim("role", "admin") }, default);
        ...
        [Fact]
        public async Task RefreshConnectionAuthAsync_NoClaims_SendsNullClaims()
        {
            var manager = CreateManager();
            RefreshAuthMessage captured = null;
            ...
            await manager.RefreshConnectionAuthAsync("conn-token-1", DateTimeOffset.UtcNow, null, default);
