...
namespace Microsoft.Azure.SignalR.Tests
{
    public class ServiceEndpointProviderFacts
    {
        ...
        private static readonly string[] QueryStringList = {"", "?name1=value1&name2=value2"};
        ...
        public static IEnumerable<object[]> PreviewEndpointProvidersWithQueryString => 
            from provider in PreviewEndpointProviderArray
            from queryString in QueryStringList
            select (new object[] {provider, queryString});

        public static IEnumerable<object[]> QueryStrings =>
            QueryStringList.Select(queryString => new object[] {queryString});
        ...
        [Theory]
        [MemberData(nameof(PreviewEndpointProvidersWithQueryString))]
        internal void GetPreviewClientEndpoint(IServiceEndpointProvider provider, string queryString)
        {
            var expected = string.IsNullOrEmpty(queryString)
                ? $"{Endpoint}:5001/client/?hub={HubName}"
                : $"{Endpoint}:5001/client/?hub={HubName}&{queryString.Substring(1)}";
            var actual = provider.GetClientEndpoint(HubName, new QueryString(queryString));
            Assert.Equal(expected, actual);
        }
        ...
        public void GetV1ClientEndpoint(string queryString)
        {
            var expected = string.IsNullOrEmpty(queryString)
                ? $"{Endpoint}/client/?hub={HubName}"
                : $"{Endpoint}/client/?hub={HubName}&{queryString.Substring(1)}";

            var actual = V1EndpointProvider.GetClientEndpoint(HubName, new QueryString(queryString));
            Assert.Equal(expected, actual);
        }
