...
{
    private static readonly Claim[] JwtAuthenticatedClaims = [new Claim("dummy", "dummy"), new Claim("name", "name"), new Claim("role", "admin"), new Claim("aud", "aud")];
    ...
    [Fact]
    public void TestCustomClaimsHasOriginalIssuer()
    {
        var originalIssuer = "microsoft.com";

        var claims = new Claim[] {
            new Claim(Constants.ClaimType.AzureSignalRUserIssuer, originalIssuer),
            new Claim(Constants.ClaimType.AzureSignalRUserPrefix + "foo", "bar"),
            new Claim(Constants.ClaimType.Id, "bar"),
            new Claim("other", "bar"),
        };

        foreach (var claim in ClaimsUtility.GetUserPrincipal(claims).Claims)
        {
            var expectedOriginalIssuer = claim.Type switch
            {
                "iss" => originalIssuer,
                "foo" => originalIssuer,
                _ => ClaimsIdentity.DefaultIssuer,
            };
            Assert.Equal(expectedOriginalIssuer, claim.OriginalIssuer);
        }
    }
    ...
    public void TestGetPreservedSystemClaims()
    {
        ...
        var claims = ClaimsUtility.BuildJwtClaims(
            new ClaimsPrincipal(new ClaimsIdentity([new Claim("iss", "A"), new Claim("jti", "B")])), null, null).ToArray();
    ...
    public void TestGetSubjectClaims()
    {
        ...
        var claims = ClaimsUtility.BuildJwtClaims(
            new ClaimsPrincipal(new ClaimsIdentity([new Claim("sub", "A"), new Claim("sub", "B")])), null, null).ToArray();
        ...
        claims = ClaimsUtility.BuildJwtClaims(
            new ClaimsPrincipal(new ClaimsIdentity([new Claim("sub", "A"), new Claim("sub", "B")])), "C", null).ToArray();
        ...
        claims = ClaimsUtility.BuildJwtClaims(
            new ClaimsPrincipal(new ClaimsIdentity([new Claim("sub", "A")])), null, null).ToArray();
        ...
        claims = ClaimsUtility.BuildJwtClaims(
            new ClaimsPrincipal(new ClaimsIdentity([new Claim("sub", "A")])), "C", null).ToArray();
        Assert.Equal("asrs.s.uid", claims[0].Type);
        Assert.Equal("sub", claims[1].Type);
