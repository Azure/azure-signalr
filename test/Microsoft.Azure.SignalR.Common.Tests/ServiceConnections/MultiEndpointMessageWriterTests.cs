...
public class MultiEndpointMessageWriterTests
{
    ...
    public async Task ListConnectionsInGroup(int? top, int resultCount, int expectedResultPage, params int?[] expectedTopsInInvocations)
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            ...
            var resultFromConnectionContainer = MockAsyncEnumerable<SignalRGroupMember>.From(new GroupMemberQueryResultPage([
                new SignalRGroupMember("1"),
                new SignalRGroupMember("2"),
                new SignalRGroupMember("3")],
                null)
            );
        ...
        var resultPages = new List<Page<SignalRGroupMember>>();
