...
internal static class ClaimsUtility
{
    ...
    {
        return message == null ? throw new ArgumentNullException(nameof(message)) : GetUserPrincipal(message.Claims);
    }

    internal static ClaimsPrincipal GetUserPrincipal(IEnumerable<Claim> messageClaims)
    {
        if (messageClaims == null || !messageClaims.Any())
        {
        ...
        var originalIssuer = messageClaims.FirstOrDefault(x => string.Equals(Constants.ClaimType.AzureSignalRUserIssuer, x.Type))?.Value;
        ...
        foreach (var claim in messageClaims)
        {
            ...
            else if (claim.Type.StartsWith(Constants.ClaimType.AzureSignalRUserPrefix))
            {
                ...
                claims.Add(new Claim(claimName, claim.Value, ClaimValueTypes.String, ClaimsIdentity.DefaultIssuer, originalIssuer));
            }
