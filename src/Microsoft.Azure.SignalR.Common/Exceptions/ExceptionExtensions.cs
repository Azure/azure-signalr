...
namespace Microsoft.Azure.SignalR.Common
{
    internal static class ExceptionExtensions
    {
        ...
        internal static Exception WrapAsAzureSignalRException(this Exception e, WebSocketConnectionOptions options)
        {
            switch (e)
            {
                case WebSocketException webSocketException:
                    if (options.AuthType == AuthType.AzureAd && e.Message.Contains("401"))
                    {
