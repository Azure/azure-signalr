    ...
    public class CloudConnection<THub> : IMessageSender where THub : Hub
    {
        ...
        public Task SendAllProtocolRawMessage(IDictionary<string, string> meta, string method, object[] args)
        {
            ...
            if (method != null)
            {
                ...
                hubInvocationMessageWrapper.Payload = _jsonProtocol.WriteToArray(message);
                hubInvocationMessageWrapper.PayloadExt = _messagePackProtocol.WriteToArray(message);
            }
        ...
        {
            switch (hubMessage.Target)
            {
                case HubInvocationServiceConstants.OnConnected:
                    await OnConnectedAsync(hubMessage);
                    break;
                case HubInvocationServiceConstants.OnDisconnected:
        ...
        private HubConnectionContext CreateHubConnectionContext(HubInvocationMessageWrapper message)
        {
            ...
            var protocolName = message.Type == TransferFormat.Binary ? MessagePackHubProtocol.ProtocolName : JsonHubProtocol.ProtocolName;
