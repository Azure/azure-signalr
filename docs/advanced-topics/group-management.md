...
TTL determines whether the **future** connections with the user ID is added to the group. To remove the already existing connections with the user ID from a group excipitly, you can call `remove a user ID from a group` API. To remove the already existing connections with the user ID from all groups excipitly, please see [Remove user from all groups](#remove-user-from-all-groups) section.
